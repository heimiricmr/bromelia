...
@include_error_bit
def create_missing_avp_response(proxy_response: type[DiameterAnswer], msg: str = None, avp: DiameterAVP = None, **kwargs) -> type[DiameterAnswer]:
    if avp is not None:
...
@include_error_bit
def create_invalid_avp_value_response(proxy_response: type[DiameterAnswer], msg: str = None, avp: DiameterAVP = None, **kwargs) -> type[DiameterAnswer]:
    if avp is not None:
...
@include_error_bit
def create_user_unknown_response(proxy_response: type[DiameterAnswer], **kwargs) -> type[DiameterAnswer]:
    return proxy_response(experimental_result=create_experimental_result_data(DIAMETER_ERROR_USER_UNKNOWN),
                          **kwargs)


@include_error_bit
def create_authentication_data_unavailable_response(proxy_response: type[DiameterAnswer], msg: str = None, avp: DiameterAVP = None, **kwargs) -> type[DiameterAnswer]:
    if avp is not None:
...
@include_error_bit
def create_unknown_serving_node_response(proxy_response: type[DiameterAnswer], **kwargs) -> type[DiameterAnswer]:
    return proxy_response(experimental_result=create_experimental_result_data(DIAMETER_ERROR_UNKOWN_SERVING_NODE),
                          **kwargs)


@include_error_bit
def create_rat_not_allowed_response(proxy_response: type[DiameterAnswer], **kwargs) -> type[DiameterAnswer]:
    return proxy_response(experimental_result=create_experimental_result_data(DIAMETER_ERROR_RAT_NOT_ALLOWED),
           **kwargs)


@include_error_bit
def create_roaming_not_allowed_response(proxy_response: type[DiameterAnswer], msg: str, **kwargs) -> type[DiameterAnswer]:
...
@include_error_bit
def create_realm_not_served_response(proxy_response: type[DiameterAnswer], msg: str, **kwargs) -> type[DiameterAnswer]:
...
@include_error_bit
def create_unknown_eps_subscription_response(proxy_response: type[DiameterAnswer], **kwargs) -> type[DiameterAnswer]:
    return proxy_response(experimental_result=create_experimental_result_data(DIAMETER_ERROR_UNKNOWN_EPS_SUBSCRIPTION),
                          **kwargs)


@include_error_bit
def create_success_response(proxy_response: type[DiameterAnswer], **kwargs) -> type[DiameterAnswer]:
