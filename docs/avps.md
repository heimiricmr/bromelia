...
For AVP handling, the `bromelia.avps` module is used. It contains several classes, that create DiameterAVP objects, parse byte string and may be extended by another implementations.
...
All DiameterAVP and DiameterAVP subclasses objects store its attributes as bytes. This design lies on the nature of Diameter protocol.
...
However it is up to the developer the AVP design in its applications. bromelia does not enforces DiameterAVP class inheritance approach, however do strongly recommend to use it.
...
>>> type(avp)
<class 'bromelia.avps.OriginHostAVP'>
```
...
```python
>>> from bromelia.avps import DiameterAVP
>>> avp = DiameterAVP(264, 40, None, "myhost.mynetwork.com")
>>> avp
<Diameter AVP: 264 [Origin-Host] MANDATORY>
>>> type(avp)
<class 'bromelia.avps.DiameterAVP'>
```
...
- [AVP unit tests](../tests/test_avps.py), `tests.test_avps`

Each class in `test_avps.py` represents a set of unit tests for a particular `avps.py` class. For instance, the first one is the TestDiameterAVP, a unittest.TestCase subclass. Each method implements a test regarding a specific DiameterAVP class feature.
...
It's possible to set the [protected bit](https://tools.ietf.org/html/rfc6733#section-3) of a DiameterAVP instance, or even of any DiameterAVP subclass instance, by calling the `.set_protected_bit()` method. However, the `test_diameter_avp__protected_bit__set_when_is_set()` unittest says if you try to set the protected bit to a DiameterAVP instance an exception will be thrown with the meaningul message "P-bit was already set".
...
The following unittests in `tests/avps.py` file present how to instantiate the ResultCodeAVP class for a different set of expected values, such as [success](https://tools.ietf.org/html/rfc6733#section-7.1.2) (e.g. or [erros](https://tools.ietf.org/html/rfc6733#section-7.1.3) (e.g. `DIAMETER_SUCCESS`, `DIAMETER_UNABLE_TO_COMPLY`,`DIAMETER_UNABLE_TO_DELIVER` codes, etc).
...
```python
>>> from bromelia.avps import DiameterAVP
>>> avp1 = DiameterAVP(264, 64, 0, "myhost.mynetwork.com")
>>> avp1
<Diameter AVP: 264 [Origin-Host] MANDATORY>
>>> avp2 = avp1.copy()
>>> avp1 == avp2
...
Let's consider you have a different byte stream which you got from a file or even from the network.
...
Traceback (most recent call last):
  ...
  File "F:\bromelia\avps.py", line 349, in load
    raise AVPParsingError("invalid bytes stream")
bromelia.exceptions.AVPParsingError: invalid bytes stream
```

You can find more information in the `tests/test_avps.py` file. There are examples for both valid and invalid ways of using the `.load()` staticmethod. Go over unittests that begins with `test_diameter_avp__load_staticmethod__parsing_` to have a better ideia.
...
In [Flavors of AVPs](#flavors-of-avps) section, two common ways to create custom DiameterAVP objects has been discussed. While one of them creates instance objects, the other is inhereted oriented. That's the best way to package your bundle of new Diameter AVPs and ship to other developers.

The *bromelia* is powered with the `3gpp_swm` asset which brings several Diameter AVPs and Diameter Messages to the [3GPP SWm application](https://www.etsi.org/deliver/etsi_ts/129200_129299/129273/16.02.00_60/ts_129273v160200p.pdf). It means there is a Diameter application stack available to be used for development of any Mobile Core Network application server such as AAA, ePDG or another NAS server.
...
Let's see an example found for Proxy-State AVP in `bromelia/avps.py` code.
...
The class needs to follow the pattern name as `<Diameter AVP Name>AVP`. It also needs to inherint first DiameterAVP class and secondly a BaseDataType subclass found in `bromelia/types.py`. For [Proxy-State AVP](https://tools.ietf.org/html/rfc6733#section-6.7.4), it has been defined in the spec as OctectString type, that is because ProxyStateAVP inherints from OctectStringType.
...
All DiameterAVP subclass should have a `code` class attribute as a 4 byte value and a constructor which calls for both DiameterAVP and the BaseDataType subclass constructors (always pass the `data` input argument).
...
Remember: this AVP is not vendor specific, therefore there is no need to include the `vendor_id` input argument in the `EnumeratedType` constructor call within the `DisconnectCausaAVP` constructor.
...
Now things get just a bit cumbersome. Not too much at all. But it worth a few warnings to get the max of the GroupedType inheritance.
